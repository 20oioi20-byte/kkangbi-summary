...
│   ├── collect-state.js         # 담당자·센터 기본값, 저장(localStorage), 공용 헬퍼
...
- 로컬 프로토타입(브라우저 `localStorage` 저장) — 배포 전 Supabase 연동 필요 (`COLLECT-DEPLOY-CHECKLIST.md` 참고)
- 완성 기능: 담당자별 실적/계획 작성 · 규칙기반 자동 취합 · 워드 작성규칙 반영 DOCX 다운로드(로고 포함) ·
  응대율 표(컬럼 드래그 리사이즈·순서변경) · 엑셀 다운로드 · 주차별 저장 이력 · 담당자/센터 관리
